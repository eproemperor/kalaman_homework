...
namespace Color {    // 终端彩色输出，使用ANSI转义码，用于调试信息的显示
...
struct TargetInfo {
    int id;                      // 目标唯一标识符
    cv::Point2f position;        // 当前位置（像素坐标）
    cv::Point2f velocity;        // 当前速度（像素/秒）
    int color;                   // 颜色：COLOR_RED, COLOR_BLUE, COLOR_UNKNOWN
    int hit_count;               // 已击中次数（需要累计到HITS_TO_KILL才能摧毁）
    bool is_active;              // 是否存活（未被摧毁）
    bool is_lost;                // 是否丢失（当前帧未检测到）
    bool is_occluded;            // 是否被遮挡（被友军或障碍物遮挡）
    bool is_enemy;               // 是否为敌方（true=敌方，false=友方）
    int age;                     // 存在帧数（从第一次检测到开始计数）
    float score;                 // 优先级得分（用于目标选择）
    ...
    /**
     * @brief 获取预测位置（简化线性模型）
     * @param time_sec 预测未来时间（秒）
     * @return 预测位置
     */
    cv::Point2f getPredictedPosition(float time_sec = 0.2f) const {
        return position + velocity * time_sec;  // 匀速运动模型
    }
};
...
struct SafetyCheckResult {
    bool is_safe_to_fire;       // 是否安全开火（综合判断）
    bool has_friendly_in_path;  // 友军是否在弹道上
    bool target_occluded;       // 目标是否被遮挡
    bool target_confident;      // 目标是否可靠（置信度高）
    float risk_level;           // 风险等级 0-1（越高越危险）
...
private:
    ...
    // ==================== 工具类 ====================
    openCV_tools cv_tools_;      // OpenCV图像处理工具
    SerialComm serial_comm_;     // 串口通信类
    bool serial_comm_ok_{false}; // 串口状态标志
    
    // ==================== 目标跟踪相关 ====================
    std::vector<std::shared_ptr<KalmanTracker>> trackers_;     // 智能指针向量，管理所有卡尔曼追踪器
    std::map<int, TargetInfo> target_info_;      // 目标信息映射表（ID -> 目标信息）
    std::map<int, int> tracker_lost_frames_;      // 连续丢失帧数（ID -> 丢失帧数）
    std::map<int, bool> tracker_destroyed_;       // 是否已摧毁（ID -> 摧毁状态）
    
    // 按颜色分类的目标ID列表
    std::vector<int> enemy_target_ids_;     // 敌方目标ID列表
    std::vector<int> friendly_target_ids_;  // 友方目标ID列表
    
    // 当前锁定目标
    int current_target_id_{-1};     // 当前目标ID，-1表示无目标
    int target_lock_counter_{0};    // 目标锁定计数器（连续帧数）
    const int LOCK_FRAMES_REQUIRED = 5;    // 锁定所需连续帧数（防止误触发）
    const int MAX_LOST_FRAMES = 30;         // 最大丢失帧数（超过则丢弃目标）
    
    // ==================== 状态信息 ====================
    int self_color_{COLOR_UNKNOWN};     // 己方颜色
    int enemy_color_{COLOR_UNKNOWN};     // 敌方颜色
    CannonInfo cannon_info_;             // 炮台信息
    bool cannon_detected_{false};        // 是否检测到炮台
    
    // ==================== 时间相关 ====================
    rclcpp::Time last_fire_time_;        // 上次开火时间
    rclcpp::Time last_output_time_;      // 上次状态输出时间
    rclcpp::Time last_safety_check_time_; // 上次安全检查时间
    
    // ==================== 统计信息 ====================
    int frame_count_{0};        // 帧计数器
    
    // ==================== 发射参数 ====================
    const int HITS_TO_KILL = 3;                      // 需要击中3次才能摧毁
    ...
    const float MAX_ANGLE_ERROR = 5.0f;              // 最大角度误差（度）
    
    // ==================== 防友伤参数 ====================
    const float FRIENDLY_RADIUS = 90.0f;               // 友军判定半径（像素）
    const float SAFE_ANGLE_MARGIN = 10.0f;             // 安全角度余量（度）
    ...
    // ==================== 平滑滤波 ====================
    float smoothed_angle_{0.0f};               // 平滑后的炮台角度
    const float SMOOTH_FACTOR = 0.3f;            // 平滑因子（0-1，越小越平滑）
    
    // ==================== 安全检测缓存 ====================
    SafetyCheckResult last_safety_check_;       // 上次安全检查结果缓存
...
public:
    ...
    VisionNode() : Node("vision_node") {
        ...
        image_sub_ = std::make_shared<ROSImageSubscriber>(
            std::shared_ptr<rclcpp::Node>(this, [](rclcpp::Node*){})  // 空删除器
        );
...
private:
    ...
    void processImage() {
        ...
        if (!cannon_detected_) {
            ...
            if (cannon_info_.radius > 0) {  // 检测到炮台
        ...
        if (cannon_detected_) {
            ...
            if (self_color_ == COLOR_RED) {
                friendly_centers = cv_tools_.FindFriendlyArmorCenters();  // 红色方找己方（红色）
            } else if (self_color_ == COLOR_BLUE) {
                friendly_centers = cv_tools_.FindFriendlyArmorCenters();  // 蓝色方找己方（蓝色）
            }
            
            // 合并所有检测到的目标，并标记敌我属性
            std::vector<std::pair<cv::Point2f, bool>> all_detections;
            for (const auto& pos : enemy_centers) {
                all_detections.push_back({pos, true});  // true表示敌方
            }
            for (const auto& pos : friendly_centers) {
                all_detections.push_back({pos, false}); // false表示友方
            }
            
            // ==================== 跟踪器管理 ====================
            updateTrackers(all_detections);      // 更新所有跟踪器（数据关联）
            updateTargetClassification();        // 更新敌我ID分类
            checkLostTargets();                  // 检查丢失目标
            checkOcclusion();                     // 检查遮挡情况
            selectTarget();                       // 选择最优目标
            
            // ==================== 安全检查 ====================
            if (current_target_id_ >= 0) {
                last_safety_check_ = performSafetyCheck();  // 执行安全检查
            }
            
            // ==================== 绘制显示 ====================
            drawResults(frame);  // 在图像上绘制所有信息
            
            cv::imshow("装甲板跟踪", frame);
            cv::waitKey(1);  // 等待1ms，允许OpenCV处理窗口事件
            
            // ==================== 状态输出 ====================
            auto now = this->now();
            if (now - last_output_time_ >= std::chrono::seconds(1)) {
                outputStatus();  // 每秒输出一次状态信息
                last_output_time_ = now;
            }
            
            // ==================== 开火决策 ====================
            if (current_target_id_ >= 0 && serial_comm_ok_) {
                fireAtTargetWithSafety();  // 带安全检查的开火决策
            }
        }
    }
    ...
    void updateTrackers(const std::vector<std::pair<cv::Point2f, bool>>& detections) {
        const double MATCH_THRESHOLD = 50.0;  // 匹配阈值（像素），超过此距离不匹配
        ...
        for (size_t i = 0; i < trackers_.size(); i++) {
            ...
            for (size_t j = 0; j < detections.size(); j++) {
                if (matched_detections[j]) continue;  // 已匹配的检测跳过
            ...
            if (best_match >= 0) {
                ...
                tracker_lost_frames_[i] = 0;  // 重置丢失帧计数
        ...
        // ==================== 第二步：添加新跟踪器 ====================
        for (size_t j = 0; j < detections.size(); j++) {
            if (!matched_detections[j]) {  // 未匹配的检测创建新跟踪器
                auto tracker = std::make_shared<KalmanTracker>();
                tracker->reset(detections[j].first);  // 初始化卡尔曼滤波器
                trackers_.push_back(tracker);
                
                int new_id = trackers_.size() - 1;  // 新ID = 索引
    ...
    void updateTargetClassification() {
        ...
        for (const auto& kv : target_info_) {
            ...
            if (tracker_destroyed_[id]) continue;  // 已摧毁的目标跳过
    ...
    bool checkFriendlyInFirePath(const cv::Point2f& target_pos) {
        ...
        direction = direction / path_length;  // 归一化为单位向量
    ...
    void selectTarget() {
        ...
        if (current_target_id_ >= 0 && 
            ...
            target_info_[current_target_id_].is_enemy) {  // 只锁定敌方目标
            
            target_lock_counter_++;  // 增加锁定计数
            return;  // 继续使用当前目标
        }
        ...
        // 评估所有有效敌方目标
        std::vector<std::pair<int, float>> candidates;  // <目标ID, 得分>
        ...
        if (!candidates.empty()) {
            ...
            current_target_id_ = candidates[0].first;  // 选择最高分
            target_lock_counter_ = 1;  // 开始锁定计数
        }
    }
    ...
    void drawResults(cv::Mat& frame) {
        ...
        for (size_t i = 0; i < trackers_.size(); i++) {
            ...
            snprintf(label, sizeof(label), 
                    ...
                    target_info_[i].age,  // 存在帧数
                    target_info_[i].confidence,  // 置信度
        ...
        snprintf(info, sizeof(info), 
                ...
                frame_count_,  // 总帧数
                enemy_target_ids_.size(),  // 敌方目标数
                friendly_target_ids_.size(),  // 友方目标数
                serial_comm_ok_ ? "OK" : "ERR",  // 串口状态
                last_safety_check_.is_safe_to_fire ? "SAFE" : "UNSAFE",  // 安全状态
    ...
    void run() {
        std::thread processing_thread([this]() {
            ...
            while (running_ && rclcpp::ok()) {
                processImage();  // 处理每一帧
                std::this_thread::sleep_for(std::chrono::milliseconds(5));  // 5ms间隔，避免CPU过载
            }
        });
        
        processing_thread.detach();  // 分离线程，让它在后台运行
    }
};
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<VisionNode>();  // 创建节点
    node->run();  // 启动处理线程
    
    rclcpp::spin(node);  // 保持节点运行，处理ROS回调
